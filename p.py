...
def add_background():
    st.markdown(
        ...
        <style>
        .stApp {
            background-image: url("https://i.imgur.com/YggEc46.png");
...
if st.button("Recommend"):
    ...
    with col8:
        ...
        with st.expander("Movie Info"):
            ...
            st.markdown(f"**Overview:** {movie_info[3]}")
            col14,col15 = st.columns([1,3])
            with col14:
                st.markdown(f"**Release Date:**")
            with col15:
                st.markdown(f"<h6 style='color:green;'>{movie_info[1]}</h6>",unsafe_allow_html=True)
            st.markdown(f"**Tagline:** {movie_info[2]}")
            col16,col17 = st.columns([1,7])
            with col16:
                st.markdown(f"**Rating:**")
            with col17:
                st.markdown(f"<h6 style='color:green;'>{movie_info[4]}</h6>",unsafe_allow_html=True)
    ...
    with col9:
        ...
        with st.expander("Movie Info"):
            ...
            st.markdown(f"**Overview:** {movie_info[3]}")
            col18,col19 = st.columns([1,3])
            with col18:
                st.markdown(f"**Release Date:**")
            with col19:
                st.markdown(f"<h6 style='color:green;'>{movie_info[1]}</h6>",unsafe_allow_html=True)
            st.markdown(f"**Tagline:** {movie_info[2]}")
            col20,col21 = st.columns([1,7])
            with col20:
                st.markdown(f"**Rating:**")
            with col21:
                st.markdown(f"<h6 style='color:green;'>{movie_info[4]}</h6>",unsafe_allow_html=True)
